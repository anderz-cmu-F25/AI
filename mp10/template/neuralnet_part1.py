...
class NeuralNet(nn.Module):
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        hidden_size = 100
        self.layer1 = nn.Linear(in_size, hidden_size)   # Input to hidden layer
        self.activation = nn.ReLU()                     # Activation function
        self.layer2 = nn.Linear(hidden_size, out_size)  # Hidden to output layer
        
        # Initialize the optimizer with the learning rate provided
    ...
    def forward(self, x):
        ...
        # Forward pass through the second layer (output layer)
        x = self.layer2(x)
    ...
    def step(self, x, y):
        ...
        # Convert the loss to a Python float and return it
        # This helps with memory management and is required for Gradescope
        return loss_value.item()  # or use float(loss_value.detach().cpu().numpy()) if needed
...
def fit(train_set,train_labels,dev_set,epochs,batch_size=100):
    ...
    out_size = 4
    learning_rate = 0.01
    net = NeuralNet(lrate=learning_rate, loss_fn=torch.nn.CrossEntropyLoss(), in_size=in_size, out_size=out_size)
