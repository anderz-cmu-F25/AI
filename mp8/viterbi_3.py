...
def get_word_type(word):
    word_types = ["NUM", "CNN", "SHORT_S", "SHORT_OTHER", "LONG_S", "LONG_OTHER"]
...
def viterbi_stepforward(i, word, prev_prob, prev_predict_tag_seq, emit_prob, trans_prob, tag_type):
    """
    ...
    for tag in prev_prob:
        if i == 0:
            ...
            max_prob = prev_prob[tag] + log(trans_prob[tag][tag]) + (log(emit_prob[tag][word]) if emit_prob[tag][word] != 0 else log(tag_type[tag][get_word_type(word)]))
            max_tag = tag
            for tag1 in prev_prob:
                curr_prob = prev_prob[tag1] + log(trans_prob[tag1][tag]) + (log(emit_prob[tag][word]) if emit_prob[tag][word] != 0 else log(tag_type[tag][get_word_type(word)]))
...
def viterbi_3(train, test):
    ...
    for tag in emit_prob:
        for word in emit_prob[tag]:
            if emit_prob[tag][word] == 1:
                hapax[tag][get_word_type(word)] += 1
                hapax_sum += 1
        for word_type in word_types:
            if hapax[tag][word_type] == 0:
                hapax[tag][word_type] = 1
                hapax_sum += 1

    # Hapax sum up to 1
    for tag in hapax:
        for word_type in word_types:
            hapax[tag][word_type] /= hapax_sum

    # Getting Initial Prob, Transition Prob, and Emission Prob
    for tag in tag_dic:
        ...
        denominator = (ttl + laplace*sum(hapax[tag].values())*(len(word_dic) + 1))
        for word in word_dic:
            cnt = emit_prob[tag][word]
            emit_prob[tag][word] = (cnt + laplace * sum(hapax[tag].values())) / denominator
    ...
    # for sen in tqdm(range(len(test))):
        ...
        for i in range(length):
            log_prob, predict_tag_seq = viterbi_stepforward(i, sentence[i], log_prob, predict_tag_seq, emit_prob, trans_prob, hapax)
