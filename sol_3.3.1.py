...
def load_shared_keys_from_file(filename):
    shared_keys = {}
    with open(filename, 'r') as f:
        lines = [int(line.strip()) for line in f if line.strip()]
        print(len(lines) % 2)
        assert len(lines) % 2 == 0, "File should contain pairs of (p, q) values"
        for i in range(0, len(lines), 2):
            p = lines[i]
            q = lines[i + 1]
            shared_keys[i // 2] = (p, q)  # use i//2 as the key index
    return shared_keys
...
if __name__ == '__main__':
    ...
    shared_keys = load_shared_keys_from_file("keys")
