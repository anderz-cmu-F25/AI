...
class Maze:
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        """
        nearest_neighbors = []
        for i in sorted(self.filter_valid_waypoints()[cur_shape], key=partial(euclidean_distance, b=cur_waypoint)):
            if cur_waypoint != i and self.is_valid_move((cur_waypoint[0], cur_waypoint[1], cur_shape), (i[0], i[1], cur_shape)):
    ...
    def is_valid_move(self, start, end):
        ...
        # Invalid shape
        if end_shape_idx not in range(len(self.alien.get_shapes())):
            return False

        # Changing shape and moving at the same time
        elif (start_x != end_x or start_y != end_y) and start_shape_idx != end_shape_idx:
            return False
        
        # Changing shape
        elif start_x == end_x and start_y == end_y:
            return not does_alien_touch_wall(self.create_new_alien(end_x, end_y, end_shape_idx), self.walls)
        
        # Moving
        else:
            return not does_alien_path_touch_wall(self.create_new_alien(start_x, start_y, start_shape_idx), self.walls, (end[0], end[1]))
    ...
    def get_neighbors(self, x, y, shape_idx):
        ...
        """
        if (x, y, shape_idx) not in self.states:
            self.states_explored += 1
            self.states.append((x, y, shape_idx))
        ...
        for end in [(x, y, shape_idx - 1), (x, y, shape_idx + 1)]:
            start = (x, y, shape_idx)
            if (start, end) not in self.move_cache:
                self.move_cache[(start, end)] = self.is_valid_move(start, end)
            if self.move_cache[(start, end)]:
