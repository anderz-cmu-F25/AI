...
def create_linear_data(num_samples, slope, intercept, x_range=[-1.0, 1.0], noise=0.1):
    start, end = x_range
    x = np.random.uniform(start, end, (num_samples, 1))
...
def get_simple_linear_features(x):
    return np.hstack((x, np.full((x.shape[0], 1), 1)))
...
def gradient_descent(get_gradient, A_init, learning_rate, num_iterations):
    for i in range(num_iterations):
        A_init = A_init - learning_rate * get_gradient(A_init)
    return A_init
...
def stochastic_gradient_descent(get_batch_gradient, A_init, learning_rate, num_epochs, data_size, batch_size):
    for i in range(num_epochs):
        batches = np.random.permutation(data_size)
        for start in range(0, data_size, batch_size):
            end = min(start + batch_size, data_size)
            batch = batches[start:end]
            A_init = A_init - learning_rate * get_batch_gradient(A_init, batch)
    return A_init
...
def get_polynomial_features(x, degree):
    ...
    while n >= 0:
        new_column = x**n
        new_x = np.hstack((new_x, new_column))
        n -= 1
    return new_x
...
def sample_near(num_samples, config, epsilon=0.1):
    return np.random.uniform(-epsilon, epsilon, (num_samples, config.shape[0])) + config
...
def estimate_ik_gradient(loss, config, num_samples):
    max_loss = 0
    for row in sample_near(num_samples, config):
        curr_loss = loss(row)
        if curr_loss > max_loss:
            max_loss = curr_loss
            max_row = row
    return max_row / np.linalg.norm(max_row)
...
def logistic_prediction(x, A, get_modified_features):
    return 1 / (1 + np.exp(-get_modified_features(x) @ A))
...
def get_logistic_regression_features(x):
    return get_polynomial_features(x, degree=4)