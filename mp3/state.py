...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        if y != 2:
            below = copy.deepcopy(self)
            below.state[y][x], below.state[y+1][x] = below.state[y+1][x], 0
            below.dist_from_start += 1
            below.h = below.compute_heuristic()
            below.zero_loc[0] += 1
        if x != 0:
            left = copy.deepcopy(self)
            left.state[y][x], left.state[y][x-1] = left.state[y][x-1], 0
            left.dist_from_start += 1
            left.h = left.compute_heuristic()
            left.zero_loc[1] -= 1
        if y != 0:
            above = copy.deepcopy(self)
            above.state[y][x], above.state[y-1][x] = above.state[y-1][x], 0
            above.dist_from_start += 1
            above.h = above.compute_heuristic()
            above.zero_loc[0] -= 1
        if x != 2:
            right = copy.deepcopy(self)
            right.state[y][x], right.state[y][x+1] = right.state[y][x+1], 0
            right.dist_from_start += 1
            right.h = right.compute_heuristic()
            right.zero_loc[1] += 1
        
        return [i for i in [below, left, above, right] if i]
    ...
    def compute_heuristic(self):
        ...
        for x in range(3):
            for y in range(3):
                ...
                if not num:
                    continue
                total += manhattan((x, y), (num % 3, num // 3))
